...
body <- dashboardBody(
  fluidRow(
    column(
      ...
      actionButton("generate","Create first Matrix"),
      uiOutput("matrixA"),
      actionButton("generate2", "Create another matrix"),
      uiOutput("matrixB"),
      actionButton("applysub","Apply to Subspaces for A"),
      actionButton("permute","Construct the Permutation for A"),
      actionButton("verify", "Verify Isomorphism"),
      actionButton("powers","Calculate Powers for A")
    ),
    column(
      width = 3,
...
server <- function(session, input, output) {
  ...
  verify.iso <- function(){
    permA <- to.perm(A)
    permB <- to.perm(B)
    output$permA <<- renderUI(permA)
    output$permB <<- renderUI(permB)
    perm.prod <- multiply(permA, permB)
    output$perm.prod <<- renderUI(perm.prod)
    mat.prod <- F4MatProd(A,B)
    output$mat.prod <<- renderUI(jax.matrix(mat.prod, name="AB"))
    prod.perm <- to.perm(mat.prod)
    output$prod.perm <<- renderUI(to.perm(mat.prod))
    is.equal <- (perm.prod == prod.perm)
    output$is.equal <<- renderUI(ifelse(is.equal, "Isomorphism checks out!", "Something went wrong!"))
  }
