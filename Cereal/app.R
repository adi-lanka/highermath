...
facs <- c(1,2,3)
#The user interface
header <- dashboardHeader(title = "Cereal Data",
                          titleWidth = 500)
sidebar <- dashboardSidebar(disable = TRUE)
body <- dashboardBody(
  fluidRow(stylesheet,
    column(width=2,
           selectInput("factor1", "Factor 1", colnames(dset)[facs]),
           selectInput("factor2", "Factor 2", colnames(dset)[facs]),
           selectInput("num1", "Numeric 1", colnames(dset)[-facs]),
           selectInput("num2", "Numeric 2", colnames(dset)[-facs]),
