...
sph.blankPlot3D <- function(){
  ...
  plot3d(NULL, NULL, NULL, xlim = c(-1.1,1.1),ylim = c(-1.1,1.1),
...
sph.showCities3D <- function(DF){
  ...
  text3d(m[1,],m[2,],m[3,],DF$Name, col = ifelse(m[2,] < 0, "red", "black"))
}
...
sph.plotRoute3D <- function(DF,cityA,cityB,cex = 1,nstop = 0,showNames=FALSE){
  ...
  m <- matrix(nrow = 3, ncol = n)
  
  #vector of x coordinates along the 50 equally spaced parts
  x <- (vA[2] + (1:n)*(vB[2] - vA[2])/n)*pi/180
  
  #vector of y coordinates along the 50 equally spaced parts
  y <- sph.merc(vA[1]) + (1:n)(sph.merc(vB[1]) - sph.merc(vA[1]))/n
  
  for(i in (1:n)) {
    long <- x[i]*180/pi
    lat <- sph.invMerc(y[i])
    m[,i] <- sph.makeXYZ(lat, long)
  }
...
sph.showCities <- function(DF){
    ...
    text(m[1,],m[2,],DF$Name, col = ifelse(m[1,] < 0, "red", "black"))
    return(NULL)
}
