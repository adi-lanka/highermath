...
body <- dashboardBody(
    fluidRow(stylesheet,
             column(width=4,
                    ...
                    textInput("newcity", "Add City"),
                    numericInput("newlat", "Latitude", value = 0),
                    numericInput("newlong", "Longitude", value = 0),
                    actionBttn("btnnewcity", "Add City", color = "success"),
                    uiOutput("msgdist"),
                    uiOutput("dirvec"),
                    uiOutput("heading"),
                    uiOutput("pole"),
...
#New data frame to keep track of all the routes
  ...
  addCity <- function(name, lat, long, DF) {
    DF <- rbind(DF, list(name, lat, long))
    return(DF)
  }
  ...
  #Add new City
  observeEvent(input$btnnewcity, {
    addCityName <<- input$newcity
    addLat <<- input$newlat
    addLong <<- input$newlong
    # sum <- input$newlat + input$newlong
    cityDF <<- addCity(addCityName, addLat, addLong, cityDF)
    updateSelectInput(session, "selstart", choices = cityDF$Name, selected = "Boston")
    updateSelectInput(session, "seldest", choices = cityDF$Name, selected = "London")
    # output$result <- renderUI(paste("lat", addLat, "sum", sum()))
    redrawMap()
  })
