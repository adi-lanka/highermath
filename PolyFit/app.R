...
body <- dashboardBody(
  fluidRow(stylesheet,
    column(width=4,
      ...
      actionBttn("btnCOVID", "Bitcoin End of Year Prices from 2010")
...
server <- function(session, input, output) {
  ...
  observeEvent(input$btnCOVID,{
    ...
    btc <- c(0.09, 0.3, 5.27, 13.3, 770.44, 313.92, 434.46, 13412.44, 38699.47, 7188.46, 29391.78)
    x <- 1:11
    output$formula <- NULL
    coeff <- fit.makePoly(x,btc)
    output$graph <- renderPlot({
      curve(fit.eval(coeff,x), from = 1, to = 11)
      points(x,btc, pch = 21, col = "red")
    })
  })
