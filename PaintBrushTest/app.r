...
server <- function(session, input, output) {
  ...
  observeEvent(input$selstart, {
    ...
    output$plot1 <- renderPlot({plot(vals$mtc$Vol., vals$mtc$Change_Percent, xlab = "Volume", ylab = "Change Percent")})
  })
  eth <- read.csv("Ethereum.csv")
  vals <- reactiveValues(mtc = eth[,c("Vol.", "Change_Percent")])
  output$plot1 <- renderPlot({plot(vals$mtc$Vol., vals$mtc$Change_Percent, xlab = "Volume", ylab = "Change Percent")})
  output$zoom <- renderPlot({plot(vals$mtc$Vol., vals$mtc$Change_Percent,xlab = "Volume", ylab = "Change Percent", xlim = c(input$plot_brush$xmin,input$plot_brush$xmax),
